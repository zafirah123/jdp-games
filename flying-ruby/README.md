...
Built with [Phaser 3](https://phaser.io/) and served from
`vendor/phaser.min.js` in this repo, with no build step.
...
You can't open `index.html` directly in a browser because the scenes are loaded
as ES modules, which browsers only allow over `http://` and not `file://`.
...
- A **magnet power-up** bubble drifts in now and then. Collect it to pull every
  on-screen ruby toward pbot for about 5 seconds.
- The round ends two ways: **crash** into a pillar or floor, or **survive the
  full 3:00** and hit "TIME'S UP!".
...
- `http://localhost:8080/?scene=GameScene` to skip the title screen
- `http://localhost:8080/?scene=GameOverScene` to jump to results with mock data

## Project structure

```text
flying-ruby/
|-- index.html               # entry point: loads local Phaser + src/main.js
|-- vendor/
|   `-- phaser.min.js        # bundled Phaser runtime committed with the game
|-- src/
...
Real art and audio are in place. Images are **WebP** and audio is **AAC/m4a**.
A few gameplay shapes are still generated at runtime in `BootScene` rather than
loaded from a file.
